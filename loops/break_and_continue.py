# how to exit from a loop using break keyword and how to skip an iteration of a
#  loop using continue

print("for loops iterate over every element in an iterable and while loops\
 continue with iterations until their stop condition is met. This is how we\
 choose where to use the two depending on the kind of functionality we aim to\
 get")

print("However, sometimes we need to have more precise control over when our\
 loops stop")

print("This is where the 'break' keyword comes in.\n break will terminate a for\
 or while loop whenever encountered")
...
print("Suppose you want to load a ship with a list of items called manifest which\
 contains tuples as elements each with the name of an item and its weight. Ideally,\
 you would want to load all items to the ship but the ship has a weight limit\
 which you should not exceed. So you need to load the items whilst checking that\
 the weight has not been exceeded and if it has, you need to stop loading\
 (stop the loop)")
print('#' * 99)
...
print("Sometimes, we also might need to skip an iteration because of some\
...
print("Suppose, for example we had a basket of food and wanted to add only those\
 ...
 encounter a non-fruit, we are to skip it and not add it to our fruits list then\
 continue looping if there are more food items")
print('#' * 99)
...
for cargo_item, cargo_weight in manifest:
    if weight >= weight_limit:
        ...
        print("adding item {}, weight {} will exceed the weight limit of {},\
         skipping item...".format(cargo_item, cargo_weight, weight_limit))
        continue
    else:
...
for headline in headlines:
    if len(news_ticker) >= 140:
        ...
        print("adding the headline will exceed the character limit by {},\
         truncating to fit...".format(len(headline) + 1 + len(news_ticker) - 140))
...
# If the number is NOT a prime number, it should print "[number] is not a prime
#  number", and a factor of that number, other than 1 and the number itself: 
