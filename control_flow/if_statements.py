...
print('#' * 79)
print("Sometimes, you have a few blocks of code which have different conditions you want executed depending on which condition executes to True.\n In this case, Python has two more clauses you can use to have your different conditions and their corresponding blocks of code executed as follows:")
print('#' * 79)
print("1. elif(short for else if) statement - This is a conditional clause that begins with the elif keyword followed by a condition to be checked then a full colon(:) and below it an indented block of code that will execute if condition evaluates to True")
print("You can have as many elif clauses as neccessary, each with its condition to check and evaluates if the other clauses above it execute to false")
print('#' * 79)
print("2. else - This clause is the last in an if statement. It will execute if all other clauses(the if's and elif's) execute to false. It does not need a condition(boolean expression) just define it using the else keyword followed by a colon(:)")
...
print("The above code would not execute as expected because to the right of the and logical operator we do not have a boolean expression, rather we have a string. This is still valid in Python(as Python will do truth value testing on this non-boolean object) but will not give the expected result.")
